...
const Home = () => {
    ...
    const loadUsers = async () => {
        const result = await axios.get(
            `https://60f2479f6d44f300177885e6.mockapi.io/users?user_type=${tabValue}&?page=${0}&limit=${5}`
        );
        result.data.reverse()
    ...
    return (
        <div className="home-container">
            <div className="container">
                {openModal && (<AddUser closeModal={setOpenModal} settabValue={settabValue} />)}
                <div className="top">
                                ...
                                {users && users.map((user) => (
                                    <tr>
                                        ...
                                        <td data-label="Details View">
                                            <Link className="btn-primary" to={`/user/${user.id}`}>Details </Link>
                                            <Link className="btn-warn" onClick={() => deleteUser(user.id)} to="/"> Delete</Link>
                                        </td>
                                    </tr>
                                ))}
